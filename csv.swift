...
//  Modified by Tom Grobbe on 10/27/17 (Updated to Swift 4)
//
...
public class CSV {
    ...
    public init(content: String?, delimiter: NSCharacterSet, encoding: UInt) throws{
        if let csvStringToParse = content{
            self.delimiter = delimiter as CharacterSet as CharacterSet as NSCharacterSet
    ...
    public convenience init(contentsOfURL url: String) throws {
        ...
        try self.init(content: csvString,delimiter:comma as NSCharacterSet, encoding:String.Encoding.utf8.rawValue)
    }
